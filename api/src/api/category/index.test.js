...
test('POST /categories 401 (admin)', async () => {
...
test('GET /categories 200 (admin)', async () => {
  const { status, body } = await request(app())
    .get(`${apiRoot}`)
...
test('GET /categories 401 (user)', async () => {
  const { status } = await request(app())
    .get(`${apiRoot}`)
...
test('GET /categories/:id 200 (admin)', async () => {
  const { status, body } = await request(app())
...
test('GET /categories/:id 401 (user)', async () => {
  const { status } = await request(app())
    .get(`${apiRoot}/${category.id}`)
    .query({ access_token: userSession })
  expect(status).toBe(401)
...
test('GET /categories/:id 404 (admin)', async () => {
  const { status } = await request(app())
...
test('PUT /categories/:id 401 (admin)', async () => {
  const { status } = await request(app())
    .put(`${apiRoot}/${category.id}`)
    .send({ access_token: adminSession })
  expect(status).toBe(401)
...
test('DELETE /categories/:id 401 (admin)', async () => {
  const { status } = await request(app())
    .delete(`${apiRoot}/${category.id}`)
    .query({ access_token: adminSession })
  expect(status).toBe(401)
